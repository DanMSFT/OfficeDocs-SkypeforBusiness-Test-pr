...
You can verify that a user's contacts have successfully been migrated to the unified contact store by running the [Test-CsUnifiedContactStore](https://technet.microsoft.com/en-us/library/jj204662\(v=ocs.15\)) cmdlet from within the Lync Server Management Shell:
...
That also means that, after assigning the user a new user services policy, you must then run the [Invoke-CsUcsRollback](https://technet.microsoft.com/en-us/library/jj204661\(v=ocs.15\)) cmdlet in order to move the user's contacts out of Exchange 2013 and back to Lync Server 2013. For example, after assigning Ken Myer a new user services policy you can then move his contacts out of the unified contact store by using the following command:
